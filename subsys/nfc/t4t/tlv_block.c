...
LOG_MODULE_REGISTER(nfc_t4t_tlv_block, CONFIG_NFC_T4T_TLV_BLOCK_LOG_LEVEL);
...
static int nfc_t4t_file_control_value_parse(struct nfc_t4t_tlv_block *file_control_tlv,
					    u8_t *value_buff)
{
	nfc_t4t_tlv_block_file_control_val *control_tlv_val;
	...
	case NDEF_FILE_CONTROL_TLV:
		...
		NFC_T4T_FILE_CONTROL_MAX_SIZE_FIELD_RANGE_VERIFY(control_tlv_val->max_file_size,
...
int nfc_t4t_file_control_tlv_parse(struct nfc_t4t_tlv_block *file_control_tlv,
				   u8_t *raw_data,
				   u16_t *len)
{
	int err;
	...
	memset(file_control_tlv, 0, sizeof(struct nfc_t4t_tlv_block));
...
void nfc_t4t_file_control_tlv_printout(u8_t num, struct nfc_t4t_tlv_block *t4t_tlv_block)
{
	LOG_INF("%d file Control TLV", num);
	switch (t4t_tlv_block->type) {
	case NDEF_FILE_CONTROL_TLV:
		LOG_INF("Type: NDEF File Control (0x%02x)", t4t_tlv_block->type);

		break;

	case PROPRIETARY_FILE_CONTROL_TLV:
		LOG_INF("Type: Proprietary File Control (0x%02x)", t4t_tlv_block->type);

		break;

 	case EXTENDED_NDEF_FILE_CONTROL_TLV:
		LOG_INF("Type: Extended NDEF File Control (0x%02x)", t4t_tlv_block->type);
	...
	struct nfc_t4t_tlv_block_file_control_val *tlv_val = &t4t_tlv_block->value;
	...
	if (tlv_val->file.content) {
		LOG_INF("NDEF file content present. Length: %d ", tlv_val->file.len);
