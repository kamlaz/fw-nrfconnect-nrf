...
static int bt_cmd_gatt_nus_exec(CborValue *it, CborEncoder *encoder)
{
	...
	if (!cbor_value_is_byte_string(it) ||
	    cbor_value_copy_byte_string(it, buf,
		    &len, it) != CborNoError) {
		return -EINVAL;
	}
...
{
	if (bt_cb->connected) {
		bt_cb->connected(addr, err);
	}
}

static void disconnected_evt(const bt_addr_le_t *addr, u8_t err)
{
	if (bt_cb->disconnected) {
		bt_cb->disconnected(addr, err);
	}
}
