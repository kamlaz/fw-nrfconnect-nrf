...
static void connected(const bt_addr_le_t *addr, u8_t err)
{
...
static void disconnected(const bt_addr_le_t *addr, u8_t reason)
{
	char addr_str[BT_ADDR_LE_STR_LEN];
...
void ble_write_thread(void)
{
	...
	for (;;) {
		...
		retcode = bt_nus_transmit(buf->data, buf->len);
		if (retcode != 0) {
			printk("Send %d bytes: ERROR\n", buf->len);
		} else {
			printk("Send %d bytes: OK\n", buf->len);
		}
